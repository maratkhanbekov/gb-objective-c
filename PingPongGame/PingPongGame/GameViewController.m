...
-(void)tableSetup {
    // Устанавливается цвет для заднего фона
    self.view.backgroundColor = [UIColor colorWithRed:100.0/255.0 green:135.0/255.0 blue:191.0/255.0 alpha:1.0];
    
    // Устанавливаем размер и фон сетки на игровом поле, которая затем добавляется на экран
...
-(void)infoSetup {
    ...
    _scoreTop.text = @"0";
    _scoreTop.font = [UIFont systemFontOfSize:40.0 weight:UIFontWeightLight]; _scoreTop.textAlignment = NSTextAlignmentCenter;
    ...
    _scoreBottom.text = @"0";
    _scoreBottom.font = [UIFont systemFontOfSize:40.0 weight:UIFontWeightLight]; _scoreBottom.textAlignment = NSTextAlignmentCenter;
    [self.view addSubview:_scoreBottom];
}

-(void)paddlesSetup {
    // Создаем объекты ракеток – верхнюю и нижнюю.
    _paddleTop = [[UIImageView alloc] initWithFrame:CGRectMake(30, 40, 90, 60)]; _paddleTop.image = [UIImage imageNamed:@"PaddleTop"]; _paddleTop.contentMode = UIViewContentModeScaleAspectFit;
    [self.view addSubview:_paddleTop];
    
    _paddleBottom = [[UIImageView alloc] initWithFrame:CGRectMake(30, SCREEN_HEIGHT - 90, 90, 60)];
    _paddleBottom.image = [UIImage imageNamed:@"PaddleBottom"]; _paddleBottom.contentMode = UIViewContentModeScaleAspectFit;
    [self.view addSubview:_paddleBottom];
}

-(void)ballSetup {
    // Создаем представление для мяча
    _ball = [[UIView alloc] initWithFrame:CGRectMake(self.view.center.x - 10, self.view.center.y - 10, 20, 20)];
    _ball.backgroundColor = [UIColor whiteColor]; _ball.layer.cornerRadius = 10;
    _ball.hidden = YES;
...
-(void)saveData {
    ...
    path = [path stringByAppendingString:@"/scores.plist"];
    NSArray *elements = [NSArray arrayWithObjects:@"1-1", @"3-1", @"4-2", @"4-2", nil];
